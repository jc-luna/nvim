...
local function activate_env(path)
	...
	if path then
		...
		vim.notify(
			"Path is not a julia environment: \n`" .. path .. "`", vim.log.levels.WARN)
			return
		end
		_activate_env(path)
	else
...
vim.lsp.config.julials = {
	...
	on_attach = function(_, bufnr)
		...
		"LspJuliaActivateEnv", activate_env, {
